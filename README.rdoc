...
My suggest is to use this project like a black box library, if your system use other types of card or game flow, the best way is encapsulate this implementation in an Adapter, Decorator, Proxy or other design patterns 
...
=== Check out the following example
	...
	if(winnerList.contains(player)) {
		//PLAYER WINNER
		//WINNER HAND: winner.getRankingEnum();
	} 
...
===== getStatsSimple's header

Columns:
 
* STATS = RankingEnum winner
* COUNT = is how many times the same ranking winner name occurs
* PERCENT = is the percentual of the COUNT compared with all executions

===== getStatsFull's header
...
* COUNT = is how many times the same game occurs
* PERCENT = is the percentual of the COUNT compared with all executions
